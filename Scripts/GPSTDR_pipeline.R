...
output_dir <- here("E:/Chapter 2 HMM/processed_data/") #location to save processed files)
processed_dir <- ("E:/Chapter 2 HMM/processed_data/") #location of processed RDS files)
shapefile_dir <- ("D:/Shapefiles/") #location of shapefiles
...
metadata <- read_csv(here("E:/Chapter 2 HMM/metadata_MASTER.csv")) # read the metadata_MASTER file (should be put in raw_data folder)
...
TDRclean <- TDRall %>%
  ...
  group_by(deployID) %>% filter(duplicated(DateTime) == FALSE) # remove duplicate lines - all duplicated in 2022 data were
# in the upper 1m so won't impact data
...
#Side step: Extraction of dive variables for each deployment/individual 

#Read in metadata_MASTER and filter columns (using Sex_0.5)
metadata <- read_csv(file = paste0(input_dir, "/Metadata_MASTER.csv")) %>% 
...
TDRpost <- readRDS(here(processed_dir,("TDR_final.RDS"))) %>% #read in most recent step4 file
  ...
  left_join(.,metadata, by="deployID") %>% #add metadata (Sex) to TDRdataset
  ungroup()


#Filter data for first trips and summarise using diveMove::diveStats!!
future::plan(multisession, workers = 8) #get ready to run in multiple sessions (this might slow down your computer so close background apps beforehand)
...
#Loop to that creates a diveMove summary datafile for all  (big_data)
templist = list()

for (i in 1:length(TDRprocess)) {
  temp <- diveMove::diveStats(TDRprocess[[i]])
  temp$deployID <- (TDRprocess[[i]]@tdr@file)
  temp$diveID <- seq.int(nrow(temp)) 
  templist[[i]] <- temp
}
TDRsummary <- data.table::rbindlist(templist) %>%
  left_join(.,metadata, by="deployID") #add sex information

###### SAVE DIVE STAT SUMMARY FILE
write.csv(TDRsummary, file = (paste0(processed_dir, "Divestats.csv")), row.names = F)


# STEP 4: Read in raw GPS data =========================================================================================================================================================================================
...
# STEP 5: Trip detection from GPS  =====================================================================================================================================================================================
...
islands500 <- read_sf(here(shapefile_dir,"Western_cape_Clip.shp")) %>% #read in the sf object for Dassen & Robben
  ...
  dplyr::select("geometry", "NAME_3") %>%  #remove unnecessary columns
  rename("Island" = "NAME_3") %>% 
  mutate(Island = case_when(Island %in% "Swartland"== 0 ~ "Robben",  #rename the polygons to the names of the islands
                            Island %in% "City of Cape Town" == 0 ~ "Dassen")) 
...
GPStrips <- readRDS(here(output_dir,"GPS_filter.RDS")) %>% #read in the GPSfilter file from step 5
  ...
  mutate(intersection = as.integer(st_intersects(geometry, islands500)), #is the point intersection with an island (number)
         island = if_else(is.na(intersection), 'NA', islands500$Island[intersection])) %>% #add the name of the island
...
islands1500 <- read_sf(here(shapefile_dir,"Western_cape_Clip.shp")) %>% #read in the sf object for Dassen & Robben
  st_transform(3857) %>% #3857=Spherical Mercator projection (m)
  st_buffer(., dist = 1500) %>% # 500m buffer in case of noisy GPS data
  st_transform(4326) %>% #transform back to latlon
  select("geometry", "NAME_3") %>%  #remove unnecessary columns
  rename("Island" = "NAME_3") %>% 
  mutate(Island = case_when(Island %in% "Swartland"== 0 ~ "Robben",  #rename the polygons to the names of the islands
                            Island %in% "City of Cape Town" == 0 ~ "Dassen")) 
...
GPStripsummary <- GPStrips %>% #use GPStrips object with numbered trip IDs
  mutate(homeintersection = as.integer(st_intersects(geometry, islands1500)), #is the point intersection with an island1500 (add island number)
         homeisland = if_else(is.na(homeintersection), 'NA', islands1500$Island[homeintersection])) %>% #if intersection, add island name
  st_drop_geometry() %>%
...
TDRfinal <- readRDS(file=here(output_dir,"TDR_final.RDS")) %>%
  dplyr::select(-c(Pressureformat, TDR_tag)) %>% #remove some unnecessary columns
...
GPSfinal <- readRDS(file=here(output_dir,"GPS_final.RDS")) %>%
  group_by(deployID) %>% arrange(DateTime) %>%
  dplyr::select(-c(Date,Time)) %>% #remove extra columns
